...
export default function AdminKeywordsPage() {
  const liveGuidesBySlug = new Map<string, (typeof guides)[number]>(guides.map((guide) => [guide.slug, guide]));
      ...
      <div className="space-y-6 p-4 sm:p-6 lg:p-8">
        <AdminCard title="Topic Inventory" description="Seed keywords, intent, category, priority, and connected tools.">
          <div className="overflow-hidden rounded-lg border border-line">
            <div className="hidden grid-cols-[1.4fr_0.8fr_0.8fr_0.8fr_0.7fr] gap-4 bg-field px-4 py-3 text-xs font-bold uppercase tracking-wide text-slate-500 lg:grid">
              <span>Keyword</span>
              <span>Intent</span>
              <span>Category</span>
            ...
            <div className="divide-y divide-line">
              {keywordTopics.map((topic) => {
                const liveGuide = liveGuidesBySlug.get(topic.slug);
                const ecosystem = liveGuide ? getGuideEcosystemBySlug(liveGuide.primaryEcosystem) : null;

                return (
                  <article key={topic.id} className="grid gap-3 px-4 py-4 lg:grid-cols-[1.4fr_0.8fr_0.8fr_0.8fr_0.7fr] lg:items-center">
                    <div className="min-w-0">
                      <h2 className="font-bold text-ink">{topic.primaryKeyword}</h2>
                    ...
                    <div className="text-sm text-slate-700">{ecosystem?.shortTitle ?? "Draft only"}</div>
